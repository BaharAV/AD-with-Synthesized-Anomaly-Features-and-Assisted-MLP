...
def run(
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        cde_list = methods["get_cde"](imagesize, device)

        models_dir = os.path.join(run_save_path, "models")
        os.makedirs(models_dir, exist_ok=True)
        for i, Cde in enumerate(cde_list):
            # torch.cuda.empty_cache()
            if Cde.backbone.seed is not None:
                utils.fix_seeds(Cde.backbone.seed, device)
            LOGGER.info(
                "Training models ({}/{})".format(i + 1, len(cde_list))
            )
            # torch.cuda.empty_cache()

            Cde.set_model_dir(os.path.join(models_dir, f"{i}"), dataset_name)
            if not test:
                i_auroc, p_auroc = Cde.train(dataloaders["training"], dataloaders["testing"])
            else:
                i_auroc, p_auroc =  Cde.test(dataloaders["training"], dataloaders["testing"], save_segmentation_images)
...
def net(
    ...
    def get_cde(input_shape, device):
        cdes = []
        for backbone_name, layers_to_extract_from in zip(
            ...
            cdes.append(cde_inst)
        return cdes

    return ("get_cde", get_cde)
