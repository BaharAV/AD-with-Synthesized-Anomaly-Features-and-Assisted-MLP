...
class CDE(torch.nn.Module):
    ...
    def load(
        ...
        **kwargs,
        ...
        self.discriminator_2 = Discriminator_2(self.target_embed_dimension, n_layers=dsc_layers, hidden=dsc_hidden)
        self.discriminator_2.to(self.device)
        ...
        self.dsc_opt_2 = torch.optim.Adam(self.discriminator_2.parameters(), lr=self.dsc_lr, weight_decay=1e-5)
        self.dsc_schl_2 = torch.optim.lr_scheduler.CosineAnnealingLR(self.dsc_opt_2, (meta_epochs - aed_meta_epochs) * gan_epochs, self.dsc_lr*.4)
    ...
    def train(self, training_data, test_data):
        ...
        if os.path.exists(ckpt_path):
            ...
            if 'discriminator' in state_dict:
                ...
                self.discriminator_2.load_state_dict(state_dict['discriminator_2'])
            ...
            scores, segmentations, features, labels_gt, masks_gt = self.predict(test_data)
            auroc, full_pixel_auroc = self._evaluate(test_data, scores, segmentations, features, labels_gt, masks_gt)
        ...
        def update_state_dict(d):
            ...
            state_dict["discriminator_2"] = OrderedDict({
                k:v.detach().cpu() 
                for k, v in self.discriminator_2.state_dict().items()})
            if self.pre_proj > 0:
    ...
    def _train_discriminator(self, input_data):
        ...
        i_iter = 0
        LOGGER.info(f"Training discriminators...")
            ...
            for i_epoch in range(self.gan_epochs):
                ...
                for data_item in input_data:
                    ...
                    if ran_choice==1:
                        ...
                        noise = torch.stack([
                            torch.normal(0, 0.02 * 1.1**(k), true_feats.shape)
                            for k in range(self.mix_noise)], dim=1).to(self.device) # (N, K, C)
                        noise = (noise * noise_one_hot.unsqueeze(-1)).sum(1)
                        fake_feats_1 = true_feats + noise
                        fake_feats = fake_feats_1

                    elif ran_choice==2:
                        noise_idxs = torch.randint(0, self.mix_noise, torch.Size([true_feats.shape[0]]))
                        noise_one_hot = torch.nn.functional.one_hot(noise_idxs, num_classes=self.mix_noise).to(self.device) # (N, K)
                        noise = torch.stack([
                            torch.normal(0, 0.01 * 1.1**(k), true_feats.shape)
                            for k in range(self.mix_noise)], dim=1).to(self.device) # (N, K, C)
                        noise = (noise * noise_one_hot.unsqueeze(-1)).sum(1)
                        fake_feats_1 = true_feats + noise
                        fake_feats = fake_feats_1
                    ...
                    scores_2 = self.discriminator_2(torch.cat([true_feats, fake_feats]))

                    true_scores = scores[:len(true_feats)]
                    fake_scores = scores[len(fake_feats):]
                    true_scores_2 = scores_2[:len(true_feats)]
                    fake_scores_2 = scores_2[len(fake_feats):]
                    ...
                    p_true_2 = (true_scores_2.detach() >= th).sum() / len(true_scores_2)
                    p_fake_2 = (fake_scores_2.detach() < -th).sum() / len(fake_scores_2)
                    true_loss_2 = torch.clip(-true_scores_2 + th, min=0)
                    fake_loss_2 = torch.clip(fake_scores_2 + th, min=0)
                    ...
                    self.logger.logger.add_scalar(f"p_true_2", p_true_2, self.logger.g_iter)
                    self.logger.logger.add_scalar(f"p_fake_2", p_fake_2, self.logger.g_iter)

                    loss = true_loss.mean() + fake_loss.mean()
                    self.logger.logger.add_scalar("loss", loss, self.logger.g_iter)
                    self.logger.step()
                    loss_2 = true_loss_2.mean() + fake_loss_2.mean()
                    self.logger.logger.add_scalar("loss_2", loss_2, self.logger.g_iter)
                    self.logger.step()
                    ...
                    loss_2 = loss_2.detach().cpu()

                    all_loss.append(loss.item())
                    all_p_true.append(p_true.cpu().item())
                    all_p_fake.append(p_fake.cpu().item())
                    all_loss_2.append(loss_2.item())
                    all_p_true_2.append(p_true_2.cpu().item())
                    all_p_fake_2.append(p_fake_2.cpu().item())
                ...
                if self.cos_lr:
                    self.dsc_schl.step()
                    self.dsc_schl_2.step()
                
                all_loss = sum(all_loss) / len(input_data)
                all_p_true = sum(all_p_true) / len(input_data)
                all_p_fake = sum(all_p_fake) / len(input_data)

                all_loss_2 = sum(all_loss_2) / len(input_data)
                all_p_true_2 = sum(all_p_true_2) / len(input_data)
                all_p_fake_2 = sum(all_p_fake_2) / len(input_data)

                cur_lr = self.dsc_opt.state_dict()['param_groups'][0]['lr']
                cur_lr_2 = self.dsc_opt_2.state_dict()['param_groups'][0]['lr'] 

                pbar_str = f"epoch:{i_epoch} loss:{round(all_loss, 5)} "
                pbar_str += f" loss_2:{round(all_loss_2, 5)} "

                pbar_str += f" lr:{round(cur_lr, 6)}"
                pbar_str += f" lr_2:{round(cur_lr_2, 6)}" 

                pbar_str += f" p_true:{round(all_p_true, 3)} p_fake:{round(all_p_fake, 3)}"
                pbar_str += f" p_true_2:{round(all_p_true_2, 3)} p_fake_2:{round(all_p_fake_2, 3)}"
    ...
    def _predict(self, images):
        ...
        with torch.no_grad():
            ...
            patch_scores = image_scores = -self.discriminator_2(features)
